...
The rebalance is based not only on engine power but also on detailed driving characteristics such as handling, stability, and traction. Most vehicles (except rally configurations) were test-driven on the Hiroshi Raceway (Short), and the results from those runs were used to create this list.  I aimed to stay strictly within the performance limits defined for each class by the original author.
